...
class Posterior_Plotter_3():
    ...
    def plot(self):
        ...
        axs[1,0].scatter(cl_array, ce0_array, color='k', s=self.params['plot.markersize'])
        ...
        axs[1,0].axvline(x = self.params['mean.mean_lum'], color = 'k', linestyle='-')

        axs[0,1].axhline(y = self.params['mean.mean_int'], color = 'k', linestyle = '-') 
        axs[0,1].axvline(x = self.params['mean.mean_env'], color = 'k', linestyle='-')
