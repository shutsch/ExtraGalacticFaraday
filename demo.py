...
def run_inference(vanilla_likelihood=False, use_z_model=True, data_set='0.1.8'):
    ...
    data = Egf.get_rm(filter_pulsars=True, version=data_set, default_error_level=0.5)
    ...
    log_amplitude_params = {'fluctuations': {'asperity': None,'flexibility': [.1, .1], 
                                             'fluctuations': [2.0, 1.], 'loglogavgslope': [-11/3, 1.],},
                            'offset': {'offset_mean': 5., 'offset_std': [1., 1.]},}
    ...
    if vanilla_likelihood:
        rm = np.array(data['rm'])
        stddev = np.array(data['rm_err'])
        
        data_domain = ift.makeDomain(ift.UnstructuredDomain((len(rm),)))
        
        rm = ift.Field(data_domain, rm)
        stddev = ift.Field(data_domain, stddev)
        
        response = Egf.SkyProjector(theta=data['theta'], phi=data['phi'],
                                    domain=sky_domain) 
    
        
       #if we are not interested in the RM but only in its sigma we do not need to include the Rm in the following line
        model = response @ galactic_model.get_model()
        #egal_model = explicit_response @ galactic_model.get_model() + emodel.get_model()
        residual = ift.Adder(-rm) @ model
        
        likelihood = ift.GaussianEnergy(inverse_covariance=ift.makeOp(stddev**(-2), sampling_dtype=float), sampling_dtype=float) @ residual
        ...
        if use_z_model:
            ...
            #vanilla_response = Egf.SkyProjector(theta=data['theta'][z_indices], phi=data['phi'][z_indices],
            ...
            gal_theta = data['theta'][~z_indices]
            gal_phi = data['phi'][~z_indices]
            ...
            likelihoods = {'explicit_likelihood': explicit_likelihood}

        else:

            gal_rm = np.array(data['rm'])
            gal_stddev = np.array(data['rm_err'])
            gal_theta = data['theta']
            gal_phi = data['phi']
        ...
        implicit_response = Egf.SkyProjector(theta=gal_theta,
                                            phi=gal_phi,
                                            domain=sky_domain)
        
        plot = ift.Plot()
        plot.add(implicit_response.adjoint(gal_rm), vmax =100, vmin=-100)
        plot.output(name = 'test.png')
        
        plot = ift.Plot()
        plot.add(implicit_response.adjoint(gal_stddev), vmax =100, vmin=0)
        plot.output(name = 'test_std.png')


        implicit_noise = Egf.SimpleVariableNoise(gal_data_domain, alpha=5, q='mode', noise_cov=gal_stddev**2).get_model()
        ...
        likelihoods.update({'implicit_likelihood': implicit_likelihood})
        scatter_pairs.update({'implicit: ': (implicit_noise, gal_stddev**2)})
    ...
    Egf.plot_prior(ift.utilities.my_sum(likelihoods.values()), 8, sky_models=sky_models, power_models=power_models, scatter_pairs=scatter_pairs, plotting_kwargs=plotting_kwargs)
...
if __name__ == '__main__':
    ...
    run_inference(vanilla_likelihood=False, use_z_model=False)
